...
class AdapterBuildingQuit : RecyclerView.Adapter<AdapterBuildingQuit.QuitHolder>() {

    var dataBuildingChurchQuit: MutableList<ModelUangMasukKeluar?> = mutableListOf()
    ...
    private fun add(modelUangMasukKeluar: ModelUangMasukKeluar) {
        dataBuildingChurchQuit.add(modelUangMasukKeluar)
        notifyItemInserted(dataBuildingChurchQuit.size)
    }
    ...
    override fun onBindViewHolder(holder: QuitHolder, position: Int) {
        holder.bind(dataBuildingChurchQuit[position])
    }

    override fun getItemCount(): Int = dataBuildingChurchQuit.size
